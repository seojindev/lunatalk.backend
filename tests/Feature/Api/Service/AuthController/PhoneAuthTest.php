...
class PhoneAuthTest extends BaseCustomTestCase
{
    ...
    // 전화 번호 없이 요청.
    public function test_service_phone_auth_전화_번호_없이_요청()
    {
        ...
        $this->withHeaders($this->getTestApiHeaders())->json('POST', '/api/v1/service/auth/phone-auth', json_decode($testPayload, true));
    }

    // 정상적인 전화 번호가 아닐때.
    public function test_service_phone_auth_정상적인_전화_번호가_아닐때()
    {
        $this->expectException(ClientErrorException::class);
    ...
    public function test_service_phone_auth_정상적인_전화_번호_자리수_아닐때()
    {
        $this->expectException(ClientErrorException::class);
    ...
    // 정상 요청.
    public function test_service_phone_auth_정상_요청()
    {
        $testPayload = '{
