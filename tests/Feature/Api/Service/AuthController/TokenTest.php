...
{
    public function test_service_token_토큰_새로고침_refresh_token_없이_요청()
    {
        ...
        $this->withHeaders($this->getTestApiHeaders())->json('POST', '/api/v1/service/auth/token-refresh', json_decode($testPayload, true));
    }

    //  정상 refresh_token 아닐때 요청.
    public function test_service_token_정상_refresh_token이_아닐때_요청()
    {
    ...
    //  정성 토큰 리프래시.
    public function test_service_token_refresh_token_정상_요청()
    {
        $response = $this->withHeaders(self::getTestApiHeaders())->postjson('/api/v1/service/auth/login', [
