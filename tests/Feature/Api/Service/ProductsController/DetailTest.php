...
//    }
    ...
    public function test_service_products_detail_uuid_에러()
    {
        $this->expectException(NotFoundHttpException::class);

        $product_uuid = '';
        $this->withHeaders($this->getTestAccessTokenHeader())->json('PUT', "/api/v1/service/products/{$product_uuid}/detail");
    }

    public function test_service_products_detail_존재_하지_않은_상품()
    {
        $this->expectException(ModelNotFoundException::class);

        $product_uuid = '12312312312-12312312312-12312312312';

        $this->withHeaders($this->getTestApiHeaders())->json('GET', "/api/v1/service/products/{$product_uuid}/detail");
    }
...
//    }
    ...
    public function test_service_products_detail_정상()
    {
        $product = Products::select()->inRandomOrder()->first()->toArray();
        $product_uuid = $product['uuid'];
